...
//     // Save payment link url to payment record
//     const savedPayment = await this.paymentModel.findOne({
//       stripePaymentIntentId: paymentIntent.id,
//     });
//     if (savedPayment) {
//       savedPayment.stripePaymentLinkUrl = paymentLink.url;
...
//     // Create the payment link with Apple Pay enabled
...
//         type: 'redirect',
//         redirect: {
//           url: `${config.frontendUrl || 'https://ronwh-website.vercel.app'}/payment-success?bookingId=${bookingId}`,
//         },
//       },
//     });
...
export class PaymentService {
  ...
  private async createStripePaymentLink(
    ...
    // Create the payment link — Apple Pay & Google Pay auto-enabled via card
    const paymentLink = await this.stripe.paymentLinks.create({
      line_items: [{ price: priceObj.id, quantity: 1 }],
